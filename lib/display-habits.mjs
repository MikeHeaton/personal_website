...
export const HABITS = Object.freeze([
  Object.freeze({ key: "dogTeeth", label: "🐕🪥" }),
  Object.freeze({ key: "bed", label: "🛏️" }),
  Object.freeze({ key: "strengthProtein", label: "💪🥤" }),
  Object.freeze({ key: "strengthRun", label: "💪🏃" }),
]);
...
function emptyRecord() {
  return Object.fromEntries(HABITS.map(({ key }) => [key, false]));
}
...
function storedRecord(value) {
  try {
    ...
    return Object.fromEntries(HABITS.map(({ key }) => [key, parsed[key] === true]));
  } catch {
    return emptyRecord();
  }
}
...
export function emptyHabitSnapshot(now = Date.now()) {
  ...
  return { dates, records: Object.fromEntries(dates.map((date) => [date, emptyRecord()])) };
}
