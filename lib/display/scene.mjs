...
export const HABIT_ICON_WIDTH = 52;

function habitIcon(key, centerX, centerY, label) {
  const common = `fill="none" stroke="${palette.ink}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"`;
  let shapes = "";
  if (key === "dogTeeth") {
    shapes = `<path d="M-32-6l7-14 9 8h12l9-8 7 14v17c0 10-9 17-22 17s-22-7-22-17Z"/><circle cx="-20" cy="2" r="2" fill="${palette.ink}" stroke="none"/><circle cx="0" cy="2" r="2" fill="${palette.ink}" stroke="none"/><path d="M-15 10q5 5 10 0M12 18l25-25M17 23L42-2M32-12l10 10M12 18l5 5"/>`;
  } else if (key === "bed") {
    shapes = `<path d="M-35 20v-34M-35 8h70v20M-35-6h18q8 0 8 8v6M-9-10h29q15 0 15 18M-35 20h70M-29 20v8M29 20v8"/>`;
  } else if (key === "strengthProtein") {
    shapes = `<path d="M-36-8v16M-30-15v30M-24-6h20M-24 6h20M-4-8v16M2-15v30M10-17h28l-4 40H14Z M14-9h20M19 23v5M29 23v5"/>`;
  } else {
    shapes = `<path d="M-36-8v16M-30-15v30M-24-6h18M-24 6h18M-6-8v16M0-15v30"/><circle cx="21" cy="-17" r="6"/><path d="M18-9l-7 15 12 7 8-12M15-3l12 2 9-8M23 13l-5 15M23 13l14 12"/>`;
  }
  const centering = key === "dogTeeth" ? '<g transform="translate(-5 0)">' : "<g>";
  return `<g class="habit-label-art" data-icon-left="${centerX - HABIT_ICON_WIDTH / 2}" data-icon-right="${centerX + HABIT_ICON_WIDTH / 2}" transform="translate(${centerX} ${centerY})" role="img" aria-label="${escapeHtml(label)}"><title>${escapeHtml(label)}</title><g transform="scale(.58)">${centering}<g ${common} aria-hidden="true">${shapes}</g></g></g></g>`;
}
