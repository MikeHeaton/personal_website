...
function weatherChart(hours, frame, options) {
  ...
  const solarMarkers = (options.sunTimes || []).flatMap((day) => [
    { kind: "sunrise", time: day?.sunrise, symbol: "☀︎" },
    { kind: "sunset", time: day?.sunset, symbol: "☾" },
  ]).map(({ kind, time, symbol }) => {
    ...
    const labelAbove = markerY - 34 >= plot.top + 14;
    const labelY = labelAbove ? markerY - 28 : markerY + 42;
    const label = solarTimeLabel(time);
    const labelAttributes = `x="${markerX.toFixed(1)}" y="${labelY.toFixed(1)}" font-size="18" font-weight="700" text-anchor="middle" font-family="Georgia, 'Times New Roman', serif"`;
    return `<g class="solar-marker" data-kind="${kind}" data-hour="${offset.toFixed(3)}" aria-label="${kind === "sunrise" ? "Sunrise" : "Sunset"} ${label}"><line x1="${markerX.toFixed(1)}" y1="${(markerY - 15).toFixed(1)}" x2="${markerX.toFixed(1)}" y2="${(markerY + 15).toFixed(1)}" stroke="#16251c" stroke-width="2"/><text ${labelAttributes} fill="${palette.paper}" stroke="${palette.paper}" stroke-width="6" stroke-linejoin="round" aria-hidden="true">${symbol} ${label}</text><text ${labelAttributes} fill="#16251c">${symbol} ${label}</text></g>`;
  }).join("");
