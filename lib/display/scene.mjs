...
export function habitsPanel(habits, frame = layout.habits, now = Date.now()) {
  ...
  const rows = HABITS.map(({ key, label }, rowIndex) => {
    ...
    const values = snapshot.dates.map((date, columnIndex) => {
      const done = snapshot.records?.[date]?.[key] === true;
      const x = frame.x + labelWidth + (columnIndex + 0.5) * columnWidth;
      return text(done ? "✓" : "X", x, baseline, {
        size: 27,
        weight: 700,
        anchor: "middle",
        fill: done ? "#16833f" : "#000000",
      });
    }).join("");
    return `<g class="habit-row" data-habit="${key}" aria-label="${label}">${rowIndex ? line(frame.x, top, frame.x + frame.width, top, "#c5c6bd") : ""}${text(label, frame.x + 16, baseline, { size: 25, weight: 700 })}${values}</g>`;
  }).join("");
