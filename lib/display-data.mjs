...
export const nextRefresh = (now = Date.now()) => (Math.floor(now / REFRESH_INTERVAL) + 1) * REFRESH_INTERVAL;
...
export async function weather(now = Date.now()) {
  if (cached && cached.slot === Math.floor(now / REFRESH_INTERVAL)) return cached.value;
  ...
  cached = { slot: Math.floor(now / REFRESH_INTERVAL), value };
  return value;
}
