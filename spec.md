...
- Two weather-chart views that alternate every 30 seconds in a 60-second native SVG animation cycle. The default/fallback view is a rolling “Next 24 Hours” chart. The second spans exactly 48 hourly points from midnight at the start of today through midnight after tomorrow in `America/Los_Angeles`, uses six-hour ticks across the full window with dated midnight labels, and shades elapsed time in neutral gray. Both use an orange temperature line and blue rainfall bars, with separate °F and inches axes. A blue line marks zero inches whenever all rainfall values in either chart window are zero. Browsers without SVG animation show only the readable rolling chart rather than overlapping both views.
...
- `lib/display/scene.mjs` defines the 1080 × 1920 layout and renders the quote, agendas, current conditions, five-day forecast, alternating 24-hour and 48-hour charts, and footer.
...
- `lib/display-data.mjs` fetches and validates bounded Open-Meteo data, selects both the rolling 24-hour and local-midnight 48-hour forecast windows, and controls weather caching and refresh clocks.
...
- `tests/display.test.mjs` covers authentication, timing, server rendering, degraded weather behavior, calendar recurrence and boundaries, escaping, conditional scrolling, both weather windows, six-hour ticks, elapsed shading, native chart alternation/fallback, zero-rain rendering, and layout bounds.
...
3. Weather and calendar displays must remain time-relative. The rolling next-24-hours forecast, local-midnight today-and-tomorrow 48-hour forecast, elapsed-time mask, and today/tomorrow agenda must follow the current time in `America/Los_Angeles`; healthy automatic updates do not require manual edits as time advances.
...
7. Before pushing, run `npm test` and `npm run build`. Visually check `/display` at 1080 × 1920 and a narrow viewport with empty, normal, and overflowing agendas plus wet and dry forecast fixtures. Inspect scroll dwell/fallback, both chart views, the 30-second switch, elapsed mask, six-hour ticks and midnight labels, both dry-rain lines, axes, and footer; confirm that only the chart area alternates and nothing clips or overlaps. Verify `/` remains unchanged. When available, verify the actual EO1 after refresh; a modern-browser screenshot alone does not prove old-device compatibility.
