...
3. Weather displays must show timely predictions relative to the current time. For example, a “next 24 hours” prediction must cover the next 24 hours as of the current time, not a stale fixed window. A self-updating implementation satisfies this requirement: no code or content update and no push is needed merely because time advances when the live weather fetch, cache, and refresh logic automatically keeps that window aligned to the current time. Maintenance should intervene only when that mechanism is absent, stale, broken, or no longer produces the required current window.
